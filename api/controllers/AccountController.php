...
class AccountController extends Controller
{
	...
	public function actionLogin()
    {
        ...
        if (!Yii::$app->user->isGuest) {
			...
			$department = \common\models\Department::find()
				->where(['company_id'=>Yii::$app->user->identity->company_id])->asArray()
				->all();
				
			return ['error_code'=>0,'message'=>'Logined','data'=>['userid'=>Yii::$app->user->identity->id,'username'=>Yii::$app->user->identity->username,'company_id'=>Yii::$app->user->identity->company_id,'department'=>$department,'username'=>Yii::$app->user->identity->username,'permission_group_id'=>$permission_group_id,'list_permission_group'=>$list_permission_group,'user_permission'=>$user_permission,'list_permission'=>$list_permissions]];
		...
		if($post = Yii::$app->request->post()){
			...
			if ($model->load(['LoginForm' => Yii::$app->request->post()]) && $model->login()) {
				...
				->where(['company_id'=>Yii::$app->user->identity->company_id])->asArray()
				->all();
				return ['error_code'=>0,'message'=>'Logined','data'=>['userid'=>Yii::$app->user->identity->id,'company_id'=>Yii::$app->user->identity->company_id,'department'=>$department,'username'=>Yii::$app->user->identity->username,'permission_group_id'=>$permission_group_id,'list_permission_group'=>$list_permission_group,'user_permission'=>$user_permission,'list_permission'=>$list_permissions]];
			} else {
